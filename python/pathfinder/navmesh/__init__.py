...
class Navmesh:
    def __init__(
        ...
        self._graphs: List[
            ...
            []
        )  # graph, where vertices are centers of polygons, edges are pairs of two incident (by edge only!) polygons
        self._nodes: List[NavmeshNode] = []
        self._groups: List[
            List[int]
        ] = []  # each group is an array of node indexes with the same group index

        vertex_map: List[List[int]] = [
            [] for v in range(len(vertices))
        ]  # index - vertex index, value - array of incident polygons
        ...
        for node in self._nodes:
            ...
            for v_num in range(len(node_vertices)):
                ...
                v: int = node_vertices[
                    v_num + 1 if v_num < len(node_vertices) - 1 else 0
                ]
                intersection: List[int] = self._get_intersection(
                    vertex_map[u], vertex_map[v]
                )
                if len(intersection) == 0:
                    print(
                        "[Someting wrong] Intersection of polygons, incident to vertices "
                        + str(u)
                        + " and "
                        + str(v)
                        + " are empty"
                    ...
                    if node_index not in intersection:
                        print(
                            "[Something wrong] Polygon "
                            + str(node_index)
                            + " does not contains in the neighborhood of tow incident vertices"
                        )
                    else:
                        for i in intersection:
                            if i != node_index:
                                node.add_neighbor(
                                    i, self._vertices[u], self._vertices[v]
                                )
                elif len(intersection) > 2:
                    print(
                        "[Someting wrong] Intersection of polygons, incident to vertices "
        ...
        for node in self._nodes:
            ...
            if g == -1:
                ...
                node.set_group(
                    new_index, new_group, self._nodes
                )  # start recursive provess
        ...
        for group in self._groups:
            graph_edges: List[
                ...
                []
            )  # each graph is an aray of pairs (n1, n2), where n1 and n2 are node (=polygon) indexes and n1 < n2
            ...
            self._graphs.append(
                NavmeshGraph(
                    [self._nodes[i].get_center() for i in graph_verts],
                    graph_verts,
                    graph_edges,
                )
            )
        ...
        if start_node is not None and finish_node is not None:
            ...
            if group_index > -1:
                ...
                for p_i in range(1, len(graph_path)):
                    ...
                    ] = self._nodes[graph_path[p_i - 1]].get_portal(graph_path[p_i])
                ...
                while i < len(raw_path) // 2:
                    ...
                    if self._triangle_area_2(portal_apex, portal_right, right) <= 0.0:
                        if (
                            self._v_equal(portal_apex, portal_right)
                            or self._triangle_area_2(portal_apex, portal_left, right)
                            > 0.0
                        ):
                            portal_right = right
                            right_index = i
                    ...
                    if not skip_next:
                        ...
                        if self._triangle_area_2(portal_apex, portal_left, left) >= 0.0:
                            if (
                                self._v_equal(portal_apex, portal_left)
                                or self._triangle_area_2(
                                ...
                                i = apex_index
                    i += 1
                if len(finall_path) == 0 or not self._v_equal(
                    finall_path[len(finall_path) - 1], raw_path[len(raw_path) - 2]
                ):
