...
class NavmeshNode:
    ...
    def __init__(
        ...
        self._normal: Tuple[float, float, float] = self._calc_average_normal(
            self._center, self._vertices
        )
        self._portals: Dict[
            int, Tuple[Tuple[float, float, float], Tuple[float, float, float]]
        ] = {}
        self._vertex_normals: List[
            Tuple[float, float, float]
        ] = self._calc_vertex_normals(self._vertices)

    def add_neighbor(
        self,
        node_index: int,
        vertex_0: Tuple[float, float, float],
        ...
        if node_index not in self._neighbors:
            ...
            print(
                "[Something wrong] Try to add neighbor node "
                + str(node_index)
                + " to the "
                + str(self._index)
                + ", but this node already exists"
            )
    ...
    def set_group(
        self, group_index: int, group_array: List[int], all_nodes
    ):  # all_nodes: List[NavmeshNode]
        if self._group == -1:
    ...
    def is_point_inside(self, point: Tuple[float, float, float]) -> bool:
        ...
        for i in range(size):
            ...
            vector: Tuple[float, float, float] = self._cross(
                v[0] - u[0],
                v[1] - u[1],
                v[2] - u[2],
                point[0] - u[0],
            ...
            d: float = (
                vector[0] * self._vertex_normals[i][0]
                + vector[1] * self._vertex_normals[i][1]
                + vector[2] * self._vertex_normals[i][2]
            )
            if d < 0.0:
                return False
        return True
    ...
    def _calc_vertex_normals(
        ...
        for i in range(size):
            ...
            # fix zero issue
            d = d if d != 0.0 else 1.0e-5

            to_return.append((vector[0] / d, vector[1] / d, vector[2] / d))
        return to_return

    def _calc_center(
        self, array: List[Tuple[float, float, float]]
    ) -> Tuple[float, float, float]:
    ...
    def _cross(
        self, a_x: float, a_y: float, a_z: float, b_x: float, b_y: float, b_z: float
    ) -> Tuple[float, float, float]:
        return (a_y * b_z - a_z * b_y, a_z * b_x - a_x * b_z, a_x * b_y - a_y * b_x)

    def _calc_average_normal(
        self,
        ...
        for p_index in range(len(points)):
            ...
            p1: Tuple[float, float, float] = points[
                p_index + 1 if p_index < len(points) - 1 else 0
            ]
            c: Tuple[float, float, float] = self._cross(
                p0[0] - center[0],
                p0[1] - center[1],
                p0[2] - center[2],
                p1[0] - center[0],
