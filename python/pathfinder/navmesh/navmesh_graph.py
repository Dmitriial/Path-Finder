...
class NavmeshGraph:
    ...
    def __init__(
        ...
        self._incident_map: Dict[
            int, List[int]
        ] = {}  # key - vertex index, value - array of indexes of incident vertices
    ...
    def _pre_start(self, target: int):
        ...
        self._vertices_h: List[float] = [
            self._get_distance(self._positions[i], self._positions[target])
            for i in range(self._vertex_count)
        ]  # calculate distance from each vertex to the target
        self._vertices_f: List[float] = [0.0] * self._vertex_count
        self._vertices_close: List[bool] = [False] * self._vertex_count
        self._vertices_parent: List[int] = [
            -1
        ] * self._vertex_count  # store here the index of the vertex, from we come here with minimum g
            ...
            if min_vertex in self._incident_map:
                for child_index in self._incident_map[min_vertex]:
                    if self._vertices_close[child_index] is False:
                        v_g: float = (
                            self._get_distance(
                                self._positions[min_vertex],
                                self._positions[child_index],
                            )
                            + self._vertices_g[min_vertex]
                        )
                        if self._vertices_parent[child_index] == -1:
                            ...
                            self._vertices_f[child_index] = (
                                v_g + self._vertices_h[child_index]
                            )
