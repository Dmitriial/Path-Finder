...
class NavmeshBVH:
    ...
    def __init__(self, nodes: List[NavmeshNode]):
        ...
        if len(nodes) == 1:
            ...
            ] = self._node.get_vertex_coordinates()
            for v in verts:
                if v[0] < x_min:
                    x_min = v[0]
                if v[0] > x_max:
                ...
                if v[2] > z_max:
                    z_max = v[2]
            self._aabb = (
                x_min - BVH_AABB_DELTA,
                y_min - BVH_AABB_DELTA,
                z_min - BVH_AABB_DELTA,
                x_max + BVH_AABB_DELTA,
                y_max + BVH_AABB_DELTA,
                z_max + BVH_AABB_DELTA,
            )
        else:
    ...
    def is_inside_aabb(self, point: Tuple[float, float, float]) -> bool:
        ...
        """
        return (
            self._aabb[0] < point[0]
            and self._aabb[1] < point[1]
            and self._aabb[2] < point[2]
            and self._aabb[3] > point[0]
            and self._aabb[4] > point[1]
            and self._aabb[5] > point[2]
        )
    ...
    def __repr__(self) -> str:
        return (
            "<object: "
            + str(self._node if self._node is None else self._node.get_index())
            + ", left: "
            + str(self._left)
            + ", right: "
            + str(self._right)
