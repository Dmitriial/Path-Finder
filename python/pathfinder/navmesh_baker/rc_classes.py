...
class Heightfield:
    def __init__(self):
        self.width = (
            0  # The width of the heightfield. (Along the x-axis in cell units.)
        )
        self.height = (
            0  # The height of the heightfield. (Along the z-axis in cell units.)
        )
        ...
        self.ch = (
            0.0  # The height of each cell. (The minimum increment along the y-axis.)
        )
...
class CompactHeightfield:
    def __init__(self):
        ...
        self.walkable_height: int = (
            0  # The walkable height used during the build of the field
        )
        self.walkable_climb: int = (
            0  # The walkable climb used during the build of the field
        )
        self.border_size: int = (
            0  # The AABB border size used during the build of the field
        )
        self.max_distance: int = (
            0  # 2 bytes  # The maximum distance value of any span within the field
        )
        self.max_regions: int = (
            0  # 2 bytes  # The maximum region id of any span within the field
        )
        self.bmin: Tuple[float, float, float] = (
            0.0,
            0.0,
            0.0,
        )  # The minimum bounds in world space [(x, y, z)]
        self.bmax: Tuple[float, float, float] = (
            0.0,
            0.0,
            0.0,
        ...
        self.dist: List[
            ...
            []
        )  # 2 bytes per element  # Array containing border distance data [Size: spanCount]
        self.areas: List[
            int
        ] = []  # 1 byte per element  # Array containing area id data [Size: spanCount]
...
class DirtyEntry:
    def __init__(
        self, _index: int = 0, _region: int = 0, _distance2: int = 0  # 2 bytes
    ):  # 2 bytes
        self.index = _index
        self.region = _region
...
class ContourSet:
    def __init__(self):
        ...
        self.max_error: float = (
            0.0  # The max edge error that this contour set was simplified with
        )
...
class PolyMesh:
    def __init__(self):
        self.verts: List[
            int
        ] = []  # 2 bytes per element  # The mesh vertices [Form: (x, y, z) * nverts]
        self.polys: List[
            int
        ] = []  # 2 bytes  # Polygon and neighbor data [Length: maxpolys * 2 * nvp]
        self.regs: List[
            int
        ] = []  # 2 bytes  # The region id assigned to each polygon [Length: maxpolys]
        self.flags: List[
            int
        ] = []  # 2 bytes  # The user defined flags for each polygon [Length: maxpolys]
        self.areas: List[
            int
        ] = []  # 1 byte  # The area id assigned to each polygon [Length: maxpolys]
        ...
        self.bmin: Typle[float, float, float] = (
            0.0,
            0.0,
            0.0,
        )  # The minimum bounds in world space [(x, y, z)]
        ...
        self.max_edge_error: float = (
            0.0  # The max error of the polygon edges in the mesh
        )
