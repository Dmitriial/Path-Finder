...
def divide_poly(
    ...
    nin: int,
    out1: int,
    out2: int,  # pointers in the buffer where we should write the data
    x: float,
    axis: int,
) -> Tuple[int, int]:  # return nout1 and nout2
    d: List[float] = [0.0] * 12
    for i in range(nin):
    ...
    for i in range(nin):
        ...
        if ina != inb:
            s: float = d[j] / (d[j] - d[i])
            buf[m * 3 + out1] = (
                buf[j * 3 + in_ptr] + (buf[i * 3 + in_ptr] - buf[j * 3 + in_ptr]) * s
            )
            buf[m * 3 + out1 + 1] = (
                buf[j * 3 + 1 + in_ptr]
                + (buf[i * 3 + 1 + in_ptr] - buf[j * 3 + 1 + in_ptr]) * s
            )
            buf[m * 3 + out1 + 2] = (
                buf[j * 3 + 2 + in_ptr]
                + (buf[i * 3 + 2 + in_ptr] - buf[j * 3 + 2 + in_ptr]) * s
            )
...
def rasterize_triangles(
    ...
    for i in range(nt):
        ...
        is_rasterize: bool = rasterize_tri(
            v0,
            v1,
            v2,
            areas[i],
