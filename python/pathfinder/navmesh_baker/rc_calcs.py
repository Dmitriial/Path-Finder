...
# return w and h
def calc_grid_size(
    bmin: Tuple[float, float, float], bmax: Tuple[float, float, float], cs: float
) -> Tuple[int, int]:
...
def v_equal(a: int, b: int, array: List[int]) -> bool:
    return array[a] == array[b] and array[a + 2] == array[b + 2]


def calc_tri_normal(
    v0: List[float], v1: List[float], v2: List[float], norm: List[float]
):  # norm is output
    e0 = [0.0, 0.0, 0.0]
    e1 = [0.0, 0.0, 0.0]
    v_sub(e0, v1, v0)
