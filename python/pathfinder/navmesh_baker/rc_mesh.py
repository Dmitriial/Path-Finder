...
from pathfinder.navmesh_baker.rc_classes import Contour, ContourSet, Edge, PolyMesh
...
def area2(array: List[int], a_ptr: int, b_ptr: int, c_ptr: int) -> int:
    return (array[b_ptr] - array[a_ptr]) * (array[c_ptr + 2] - array[a_ptr + 2]) - (
        array[c_ptr] - array[a_ptr]
    ) * (array[b_ptr + 2] - array[a_ptr + 2])


def collinear(array: List[int], a_ptr: int, b_ptr: int, c_ptr: int) -> bool:
...
def left(array: List[int], a_ptr: int, b_ptr: int, c_ptr: int) -> bool:
    return area2(array, a_ptr, b_ptr, c_ptr) < 0


def left_on(array: List[int], a_ptr: int, b_ptr: int, c_ptr: int) -> bool:
    return area2(array, a_ptr, b_ptr, c_ptr) <= 0


def intersect_prop(
    array: List[int], a_ptr: int, b_ptr: int, c_ptr: int, d_ptr: int
) -> bool:
    if (
        collinear(array, a_ptr, b_ptr, c_ptr)
        or collinear(array, a_ptr, b_ptr, d_ptr)
        or collinear(array, c_ptr, d_ptr, a_ptr)
        or collinear(array, c_ptr, d_ptr, b_ptr)
    ):
        return False
    return xorb(
        left(array, a_ptr, b_ptr, c_ptr), left(array, a_ptr, b_ptr, d_ptr)
    ) and xorb(left(array, c_ptr, d_ptr, a_ptr), left(array, c_ptr, d_ptr, b_ptr))


def between(array: List[int], a_ptr: int, b_ptr: int, c_ptr: int) -> bool:
    if not collinear(array, a_ptr, b_ptr, c_ptr):
        return False
    if array[a_ptr] != array[b_ptr]:
        return ((array[a_ptr] <= array[c_ptr]) and (array[c_ptr] <= array[b_ptr])) or (
            (array[a_ptr] >= array[c_ptr]) and (array[c_ptr] >= array[b_ptr])
        )
    else:
        return (
            (array[a_ptr + 2] <= array[c_ptr + 2])
            and (array[c_ptr + 2] <= array[b_ptr + 2])
        ) or (
            (array[a_ptr + 2] >= array[c_ptr + 2])
            and (array[c_ptr + 2] >= array[b_ptr + 2])
        )
...
def intersect(array: List[int], a_ptr: int, b_ptr: int, c_ptr: int, d_ptr: int) -> bool:
    if intersect_prop(array, a_ptr, b_ptr, c_ptr, d_ptr):
        return True
    elif (
        between(array, a_ptr, b_ptr, c_ptr)
        or between(array, a_ptr, b_ptr, d_ptr)
        or between(array, c_ptr, d_ptr, a_ptr)
        or between(array, c_ptr, d_ptr, b_ptr)
    ):
        return True
    else:
        return False
...
def diagonalie(i: int, j: int, n: int, verts: List[int], indices: List[int]) -> bool:
    ...
    for k in range(n):
        ...
        if not ((k == i) or (k1 == i) or (k == j) or (k1 == j)):
            ...
            if (
                v_equal(d0_ptr, p0_ptr, verts)
                or v_equal(d1_ptr, p0_ptr, verts)
                or v_equal(d0_ptr, p1_ptr, verts)
                or v_equal(d1_ptr, p1_ptr, verts)
            ):
                continue
...
def in_cone(i: int, j: int, n: int, verts: List[int], indices: List[int]) -> bool:
...
def diagonal(i: int, j: int, n: int, verts: List[int], indices: List[int]) -> bool:
    return in_cone(i, j, n, verts, indices) and diagonalie(i, j, n, verts, indices)


def diagonalie_loose(
    i: int, j: int, n: int, verts: List[int], indices: List[int]
) -> bool:
    ...
    for k in range(n):
        ...
        if not ((k == i) or (k1 == i) or (k == j) or (k1 == j)):
            ...
            if (
                v_equal(d0, p0, verts)
                or v_equal(d1, p0, verts)
                or v_equal(d0, p1, verts)
                or v_equal(d1, p1, verts)
            ):
                continue
...
def in_cone_loose(i: int, j: int, n: int, verts: List[int], indices: List[int]) -> bool:
    pi: int = (indices[i] & 0x0FFFFFFF) * 4
    pj: int = (indices[j] & 0x0FFFFFFF) * 4
    pi1: int = (indices[next(i, n)] & 0x0FFFFFFF) * 4
    pin1: int = (indices[prev(i, n)] & 0x0FFFFFFF) * 4
...
def diagonal_loose(
    ...
    return in_cone_loose(i, j, n, verts, indices) and diagonalie_loose(
        i, j, n, verts, indices
    )


def triangulate(n: int, verts: List[int], indices: List[int], tris: List[int]) -> int:
    ntris: int = 0
...
def compute_vertex_hash(x: int, y: int, z: int) -> int:
...
def add_vertex(
    x: int,  # all of then insigned 2 bytes
    y: int,
    z: int,
    verts: List[int],
    first_vert: List[int],
    next_vert: List[int],
    nv: int,
) -> Tuple[int, int]:  # unsigned 2 bytes, also return nv
...
def count_poly_verts(array: List[int], p: int, nvp: int) -> int:
    for i in range(nvp):
...
def uleft(
    array: List[int], a: int, b: int, c: int  # all three unsigned 2 bytes
) -> bool:
    return (array[b] - array[a]) * (array[c + 2] - array[a + 2]) - (
        array[c] - array[a]
    ) * (array[b + 2] - array[a + 2]) < 0


def get_poly_merge_value(
    polys: List[int], pa: int, pb: int, verts: List[int], ea: int, eb: int, nvp: int
) -> Tuple[int, int, int]:  # also return ea, eb
...
def can_remove_vertex(mesh: PolyMesh, rem: int) -> bool:
    nvp: int = mesh.nvp
...
def push_back(v: int, array: List[int], an: int) -> int:  # return an
    array[an] = v
    an += 1
    return an


def push_front(v: int, arr: List[int], an: int) -> int:  # return an
    an += 1
...
def remove_vertex(mesh: PolyMesh, rem: int, max_tris: int) -> bool:
    nvp: int = mesh.nvp
    ...
    for j in range(ntris):
        ...
        if (
            ...
            # mark it as such
            if (
                hreg[tris[t]] != hreg[tris[t + 1]]
                or hreg[tris[t + 1]] != hreg[tris[t + 2]]
            ):
        ...
        while True:
            ...
            for j in range(npolys - 1):
                ...
                for k in range(j + 1, npolys):
                    ...
                    ea = 0
                    eb = 0
                    v, ea, eb = get_poly_merge_value(
                        polys, pj, pk, mesh.verts, ea, eb, nvp
                    )
                    if v > bestMergeVal:
                        bestMergeVal = v
                        bestPa = j
    ...
    for i in range(npolys):
        ...
        if mesh.npolys > max_tris:
            print(
                "[Navmesh Baker]: remove_vertex: Too many polygons "
                + str(mesh.npolys)
                + " (max: "
                + str(max_tris)
                + ")."
...
def build_mesh_adjacency(
    polys: List[int], npolys: int, nverts: int, verts_per_poly: int
) -> bool:
    ...
    for i in range(npolys):
        ...
        for j in range(verts_per_poly):
            ...
            v1 = (
                polys[t]
                if (j + 1 >= verts_per_poly or polys[t + j + 1] == RC_MESH_NULL_IDX)
                else polys[t + j + 1]
            )
            if v0 < v1:
...
def build_poly_mesh(cset: ContourSet, nvp: int, mesh: PolyMesh) -> bool:
    ...
    for i in range(cset.nconts):
        ...
        for j in range(cont.nverts):
            indices[j], mesh.nverts = add_vertex(
                cont.verts[j * 4],
                cont.verts[j * 4 + 1],
                cont.verts[j * 4 + 2],
                mesh.verts,
        ...
        for j in range(ntris):
            if (
                tris[j * 3] != tris[j * 3 + 1]
                and tris[j * 3] != tris[j * 3 + 2]
                and tris[j * 3 + 1] != tris[j * 3 + 2]
            ):
            ...
            while True:
                ...
                while j < npolys - 1:
                    ...
                    while k < npolys:
                        ...
                        eb: int = 0
                        (v, ea, eb) = get_poly_merge_value(
                            polys, pj_ptr, pk_ptr, mesh.verts, ea, eb, nvp
                        )
                        if v > bestMergeVal:
                            bestMergeVal = v
                            bestPa = j
                ...
                if bestMergeVal > 0:
                    ...
                    merge_poly_verts(
                        polys, pa_ptr, pb_ptr, bestEa, bestEb, tmp_poly, nvp
                    )
    ...
    while i < mesh.nverts:
        if vflags[i] > 0:
            ...
            if not remove_vertex(mesh, i, max_tris):
                ...
                print(
                    "[Navmesh Baker] build_poly_mesh: Failed to remove edge vertex "
                    + str(i)
                )
                return False
            # Remove vertex
    ...
    if mesh.nverts > 0xFFFF:
        print(
            "[Navmesh Baker] build_poly_mesh: The resulting mesh has too many vertices "
            + str(mesh.nverts)
            + " (max "
            + str(0xFFFF)
    ...
    if mesh.npolys > 0xFFFF:
        print(
            "[Navmesh Baker] build_poly_mesh: The resulting mesh has too many polygons "
            + str(mesh.npolys)
            + " (max "
            + str(0xFFFF)
            + "). Data can be corrupted"
