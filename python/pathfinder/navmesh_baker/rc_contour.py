...
def get_corner_height(
    x: int, y: int, i: int, dir: int, chf: CompactHeightfield, is_border_vertex: bool
) -> Tuple[int, bool]:  # also return is_border_vertex
    ...
    if s is not None:
        ...
        for j in range(4):
            ...
            two_same_exts: bool = (regs[av] & regs[bv] & RC_BORDER_REG) != 0 and regs[
                av
            ] == regs[bv]
            two_ints: bool = ((regs[cv] | regs[dv]) & RC_BORDER_REG) == 0
            ints_same_area: bool = (regs[cv] >> 16) == (regs[dv] >> 16)
            no_zeros: bool = (
                regs[av] != 0 and regs[bv] != 0 and regs[cv] != 0 and regs[dv] != 0
            )
...
def walk_contour(
    x: int, y: int, i: int, chf: CompactHeightfield, flags: List[int], points: List[int]
):
    ...
    while iter < 40000:
        ...
        if flags[i] & (1 << dir):
            ...
            px: int = x
            py, is_border_vertex = get_corner_height(
                x, y, i, dir, chf, is_border_vertex
            )
            pz: int = y
            if dir == 0:
                pz += 1
...
def distance_pt_seg(x: int, z: int, px: int, pz: int, qx: int, qz: int) -> float:
...
def simplify_contour(
    ...
    if has_connections:
        ...
        while i < ni:
            ii = (i + 1) % ni
            different_regs: bool = (points[i * 4 + 3] & RC_CONTOUR_REG_MASK) != (
                points[ii * 4 + 3] & RC_CONTOUR_REG_MASK
            )
            area_borders: bool = (points[i * 4 + 3] & RC_AREA_BORDER) != (
    ...
    while i < len(simplified) // 4:
        ...
        if (points[ci * 4 + 3] & RC_CONTOUR_REG_MASK) == 0 or (
            points[ci * 4 + 3] & RC_AREA_BORDER
        ):
            while ci != endi:
                d: float = distance_pt_seg(
    ...
    if (
        max_edge_len > 0
        and (build_flags & (RC_CONTOUR_TESS_WALL_EDGES | RC_CONTOUR_TESS_AREA_EDGES))
        != 0
    ):
        i = 0
        while i < len(simplified) // 4:
            ...
            tess: bool = False
            # Wall edges
            if (build_flags & RC_CONTOUR_TESS_WALL_EDGES) and (
                points[ci * 4 + 3] & RC_CONTOUR_REG_MASK
            ) == 0:
                tess = True
            # Edges between areas
            if (build_flags & RC_CONTOUR_TESS_AREA_EDGES) and (
                points[ci * 4 + 3] & RC_AREA_BORDER
            ):
                tess = True
...
def calc_area_of_polygon_2d(verts: List[int], nverts: int) -> int:
    area: int = 0
    j: int = nverts - 1
...
def compare_holes(a: ContourHole, b: ContourHole) -> int:
    if a.minx == b.minx:
        if a.minz < b.minz:
            return -1
        if a.minz > b.minz:
...
def area2(array: List[int], a: int, b: int, c: int) -> int:
    return (array[b] - array[a]) * (array[c + 2] - array[a + 2]) - (
        array[c] - array[a]
    ) * (array[b + 2] - array[a + 2])


def left_on(array: List[int], a: int, b: int, c: int) -> bool:
    return area2(array, a, b, c) <= 0


def left(
    a: int, a_array: List[int], b: int, b_array: List[int], c: int, c_array: List[int]
) -> bool:
    v: int = (b_array[b] - a_array[a]) * (c_array[c + 2] - a_array[a + 2]) - (
        c_array[c] - a_array[a]
    ) * (b_array[b + 2] - a_array[a + 2])
    return v < 0
...
def in_cone(
    ...
    v1: int = (p_array[pj] - verts[pi]) * (verts[pin1 + 2] - verts[pi + 2]) - (
        verts[pin1] - verts[pi]
    ) * (p_array[pj + 2] - verts[pi + 2])
    v2: int = (verts[pi] - p_array[pj]) * (verts[pi1 + 2] - p_array[pj + 2]) - (
        verts[pi1] - p_array[pj]
    ) * (verts[pi + 2] - p_array[pj + 2])
    if left_on(verts, pin1, pi, pi1):
        return v1 < 0 and v2 < 0
    return not (v1 <= 0 and v2 <= 0)


def compare_diag_dist(a: PotentialDiagonal, b: PotentialDiagonal) -> int:
    if a.dist < b.dist:
        return -1
    if a.dist > b.dist:
        return 1
...
def intersect_prop(
    ...
    if (
        collinear(a, a_array, b, b_array, c, c_array)
        or collinear(a, a_array, b, b_array, d, d_array)
        or collinear(c, c_array, d, d_array, a, a_array)
        or collinear(c, c_array, d, d_array, b, b_array)
    ):
        return False
    return xorb(
        left(a, a_array, b, b_array, c, c_array),
        left(a, a_array, b, b_array, d, d_array),
    ) and xorb(
        left(c, c_array, d, d_array, a, a_array),
        left(c, c_array, d, d_array, b, b_array),
    )
...
def between(
    ...
    if a_array[a] != b_array[b]:
        return ((a_array[a] <= c_array[c]) and (c_array[c] <= b_array[b])) or (
            (a_array[a] >= c_array[c]) and (c_array[c] >= b_array[b])
        )
    else:
        return (
            (a_array[a + 2] <= c_array[c + 2]) and (c_array[c + 2] <= b_array[b + 2])
        ) or ((a_array[a + 2] >= c_array[c + 2]) and (c_array[c + 2] >= b_array[b + 2]))
...
def intersect(
    ...
    if intersect_prop(a, a_array, b, b_array, c, c_array, d, d_array):
        ...
        between(a, a_array, b, b_array, c, c_array)
        or between(a, a_array, b, b_array, d, d_array)
        or between(c, c_array, d, d_array, a, a_array)
        or between(c, c_array, d, d_array, b, b_array)
    ):
        return True
    else:
        return False
...
def intersect_seg_countour(
    ...
    for k in range(n):
        ...
        p1: int = k1 * 4
        if (
            (d0_array[d0] == verts[p0] and d0_array[d0 + 2] == verts[p0 + 2])
            or (d1_array[d1] == verts[p0] and d1_array[d1 + 2] == verts[p0 + 2])
            or (d0_array[d0] == verts[p1] and d0_array[d0 + 2] == verts[p1 + 2])
            or (d1_array[d1] == verts[p1] and d1_array[d1 + 2] == verts[p1 + 2])
        ):
            continue
...
def merge_contours(ca: Contour, cb: Contour, ia: int, ib: int) -> bool:
...
def merge_region_holes(region: ContourRegion, holes: List[ContourHole]):
    ...
    sorted_holes: List[ContourHole] = sorted(
        holes[region.holes_index : region.holes_index + region.nholes],
        key=cmp_to_key(compare_holes),
    )
    for i in range(region.nholes):
            ...
            if hole is not None:
                for iter in range(hole.nverts):
                    ...
                    for j in range(outline.nverts):
                        if in_cone(
                            j, outline.nverts, outline.verts, corner, hole.verts
                        ):
                    ...
                    sorted_diags: List[PotentialDiagonal] = sorted(
                        diags[0:ndiags], key=cmp_to_key(compare_diag_dist)
                    )
                    ...
                    for j in range(ndiags):
                        ...
                        intersect: bool = intersect_seg_countour(
                            pt,
                            outline.verts,
                            corner,
                            hole.verts,
                        ...
                        while k < region.nholes and not intersect:
                            ...
                            if contour is not None:
                                intersect |= intersect_seg_countour(
                                    pt,
                                    outline.verts,
                                    corner,
                                    hole.verts,
                ...
                if index == -1:
                    print(
                        "[Navmesh Baker] merge_holes: Failed to find merge points for "
                        + str(region.outline)
                        + " and "
                        + str(hole)
                        + "."
                ...
                if not merge_contours(outline, hole, index, bestVertex):
                    print(
                        "[Navmesh Baker] merge_holes: Failed to merge contours "
                        + str(region.outline)
                        + " and "
                        + str(hole)
                        + "."
...
def build_contours(
    ...
    max_edge_len: int,
    cset: ContourSet,
    build_flags: int = RC_CONTOUR_TESS_WALL_EDGES,
) -> bool:
    w: int = chf.width
    h: int = chf.height
            ...
            if c is not None:
                for i in range(c.index, c.index + c.count):
                    ...
                    if s is not None:
                        ...
                        simplify_contour(
                            verts, simplified, max_error, max_edge_len, build_flags
                        )
                        ...
                        if len(simplified) // 4 >= 3:
                            if cset.nconts > max_contours:
                                ...
                                max_contours *= 2
                                new_conts: List[Contour] = [
                                    Contour() for _ in range(max_contours)
                                ]
    ...
    if cset.nconts > 0:
        ...
        for i in range(cset.nconts):
            ...
            winding[i] = (
                -1 if calc_area_of_polygon_2d(cont1.verts, cont1.nverts) < 0 else 1
            )
            if winding[i] < 0:
                nholes += 1
        ...
        if nholes > 0:
            ...
            for i in range(cset.nconts):
                ...
                if winding[i] > 0:
                    if regions[cont2.reg].outline:
                        print(
                            "[Navmesh Baker] build_contours: Multiple outlines for region "
                            + str(cont2.reg)
                        )
            ...
            for i in range(nregions):
                ...
                if c_reg.outline is not None:
                    ...
                    print(
                        "[Navmesh Baker] build_contours: Bad outline for region "
                        + str(i)
