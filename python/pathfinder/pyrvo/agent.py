...
from pathfinder.pyrvo.utilities import (
    INFINITY,
    RVO_EPSILON,
    abs_sq,
    det,
    dist_sq_point_line_segment,
    dot,
    line_to_direction,
    line_to_point,
    normalize,
...
def linear_program1(
    ...
    for i in range(lineNo):
        ...
        numerator: float = det(
            line_i_direction, vector_difference(line_no_point, line_i_point)
        )
    ...
    if directionOpt:
        ...
        t = dot(
            line_to_direction(lines[lineNo]),
            vector_difference(optVelocity, line_no_point),
        )
...
def linear_program2(
    ...
    for i in range(len(lines)):
        if (
            det(
                (lines[i][2], lines[i][3]),
                vector_difference((lines[i][0], lines[i][1]), result),
            )
            > 0.0
        ):
            ...
            tempResult = result
            (a, b) = linear_program1(
                lines, i, radius, opt_velocity, direction_opt, result, is_log
            )
            result = b
            if not a:
                result = tempResult
...
def linear_program3(
    ...
    for i in range(beginLine, len(lines)):
        ...
        if det(line_i_direction, vector_difference(line_i_point, result)) > distance:
            ...
            projLines: List[Tuple[float, float, float, float]] = [
                (0.0, 0.0, 0.0, 0.0)
            ] * numObstLines
            ...
            (a, b) = linear_program2(
                projLines,
                radius,
                (-line_i_direction[1], line_i_direction[0]),
                True,
                result,
                is_log,
            )
...
class Agent:
    def __init__(
        ...
        self._max_neighbors = (
            default_agent._max_neighbors if max_neighbors is None else max_neighbors
        )
        self._max_speed = default_agent._max_speed if max_speed is None else max_speed
        self._neighbor_dist = (
            default_agent._neighbor_dist if neighbor_dist is None else neighbor_dist
        )
        self._time_horizon = (
            default_agent._time_horizon if time_horizon is None else time_horizon
        )
        self._time_horizon_obstacle = (
            default_agent._time_horizon_obstacle
            if time_horizon_obst is None
            else time_horizon_obst
        )
        ...
        self._orcaLines: List[
            Tuple[float, float, float, float]
        ] = []  # each line is two vectors: point and direction
    ...
    def compute_neighbors(self):
        ...
        range_sq: float = (
            self._time_horizon_obstacle * self._max_speed + self._radius
        ) ** 2
    ...
    def compute_new_velocity(self, delta_time: float):
        ...
        for i in range(len(self._obstacle_neighbors)):
            ...
            for j in range(len(self._orcaLines)):
                ...
                if (
                    det(
                        vector_difference(
                            vector_scale(invTimeHorizonObst, relativePosition1), p
                        ),
                        d,
                    )
                    - invTimeHorizonObst * self._radius
                    >= -RVO_EPSILON
                    and det(
                        vector_difference(
                            vector_scale(invTimeHorizonObst, relativePosition2), p
                        ),
                        d,
                    )
                    - invTimeHorizonObst * self._radius
            ...
            obstacleVector = vector_difference(
                obstacle2.get_point(), obstacle1.get_point()
            )
            s: float = dot(vector_negate(relativePosition1), obstacleVector) / abs_sq(
                obstacleVector
            )
            distSqLine: float = abs_sq(
                vector_difference(
                    vector_negate(relativePosition1), vector_scale(s, obstacleVector)
                )
            )
            ...
            if s < 0.0 and distSq1 <= radiusSq:
                ...
                if (
                    obstacle2.get_is_convex()
                    and det(relativePosition2, obstacle2.get_unit_dir()) >= 0.0
                ):
                    line[0] = 0.0
                    line[1] = 0.0
            ...
            if s < 0.0 and distSqLine <= radiusSq:
                ...
                leftLegDirection = vector_scale(
                    1.0 / distSq1,
                    (
                        relativePosition1[0] * leg1
                        - relativePosition1[1] * self._radius,
                        relativePosition1[0] * self._radius
                        + relativePosition1[1] * leg1,
                    ),
                )
                rightLegDirection = vector_scale(
                    1.0 / distSq1,
                    (
                        relativePosition1[0] * leg1
                        + relativePosition1[1] * self._radius,
                ...
                leftLegDirection = vector_scale(
                    1.0 / distSq2,
                    (
                        relativePosition2[0] * leg2
                        - relativePosition2[1] * self._radius,
                        relativePosition2[0] * self._radius
                        + relativePosition2[1] * leg2,
                    ),
                )
                rightLegDirection = vector_scale(
                    1.0 / distSq2,
            ...
            if (
                obstacle1.get_is_convex()
                and det(leftLegDirection, vector_negate(leftNeighbor.get_unit_dir()))
                >= 0.0
            ):
            ...
            if (
                obstacle2.get_is_convex()
                and det(rightLegDirection, obstacle2.get_unit_dir()) <= 0.0
            ):
            ...
            leftCutoff = vector_scale(
                invTimeHorizonObst,
                vector_difference(obstacle1.get_point(), self._position),
            )
            rightCutoff = vector_scale(
                invTimeHorizonObst,
                vector_difference(obstacle2.get_point(), self._position),
            )
            ...
            t: float = (
                ...
                else dot(vector_difference(self._velocity, leftCutoff), cutoffVec)
                / abs_sq(cutoffVec)
            )
            tLeft: float = dot(
                vector_difference(self._velocity, leftCutoff), leftLegDirection
            )
            tRight: float = dot(
                vector_difference(self._velocity, rightCutoff), rightLegDirection
            )

            if (t < 0.0 and tLeft < 0.0) or (
                obstacle1 == obstacle2 and tLeft < 0.0 and tRight < 0.0
            ):
                ...
                line[3] = -unitW[0]
                p = vector_sum(
                    leftCutoff, vector_scale(self._radius * invTimeHorizonObst, unitW)
                )
                line[0] = p[0]
                line[1] = p[1]
                self._orcaLines.append((line[0], line[1], line[2], line[3]))
                ...
                line[3] = -unitW[0]
                p = vector_sum(
                    rightCutoff, vector_scale(self._radius * invTimeHorizonObst, unitW)
                )
                line[0] = p[0]
                line[1] = p[1]
                self._orcaLines.append((line[0], line[1], line[2], line[3]))
            ...
            distSqCutoff: float = (
                ...
                if (t < 0.0 or t > 1.0 or obstacle1 == obstacle2)
                    ...
                    vector_difference(
                        self._velocity,
                        vector_sum(leftCutoff, vector_scale(t, cutoffVec)),
                    )
                )
            )
            distSqLeft: float = (
                ...
                if (tLeft < 0.0)
                    ...
                    vector_difference(
                        self._velocity,
                        (vector_sum(leftCutoff, vector_scale(tLeft, leftLegDirection))),
                    )
                )
            )
            distSqRight: float = (
                ...
                if (tRight < 0.0)
                    ...
                    vector_difference(
                        self._velocity,
                        vector_sum(
                            rightCutoff, vector_scale(tRight, rightLegDirection)
                        ),
                    )
                )
            )
            ...
            if distSqCutoff <= distSqLeft and distSqCutoff <= distSqRight:
                ...
                p = vector_sum(
                    leftCutoff,
                    vector_scale(self._radius * invTimeHorizonObst, (-d[1], d[0])),
                )
                line[0] = p[0]
                line[1] = p[1]
                line[2] = d[0]
                ...
                p = vector_sum(
                    leftCutoff,
                    vector_scale(
                        self._radius * invTimeHorizonObst,
                        (-leftLegDirection[1], leftLegDirection[0]),
                    ),
                )
                line[0] = p[0]
                line[1] = p[1]
                ...
                p = vector_sum(
                    ...
                    vector_scale(
                        self._radius * invTimeHorizonObst,
                        (rightLegDirection[1], -rightLegDirection[0]),
                    ),
                )
                line[0] = p[0]
                line[1] = p[1]
        ...
        for i in range(len(self._agent_neighbors)):
            ...
            if distSq > combinedRadiusSq:
                # No collision.
                w = vector_difference(
                    relativeVelocity, vector_scale(invTimeHorizon, relativePosition)
                )
                ...
                if (
                    dotProduct1 < 0.0
                    and (dotProduct1**2) > combinedRadiusSq * wLengthSq
                ):
                    ...
                    if det(relativePosition, w) > 0.0:
                        # Project on left leg.
                        d = (
                            relativePosition[0] * leg
                            - relativePosition[1] * combinedRadius,
                            relativePosition[0] * combinedRadius
                            + relativePosition[1] * leg,
                        )
                        line[2] = d[0] / distSq
                        line[3] = d[1] / distSq
                    else:
                    ...
                    u = vector_difference(
                        (dotProduct2 * line[2], dotProduct2 * line[3]), relativeVelocity
                    )
            else:
                ...
                # Vector from cutoff center to relative velocity.
                w = vector_difference(
                    relativeVelocity, vector_scale(invTimeStep, relativePosition)
                )
        ...
        if lineFail < len(self._orcaLines):
            self._new_velocity = linear_program3(
                self._orcaLines,
                numObstLines,
                lineFail,
                self._max_speed,
    ...
    def insert_agent_neighbor(self, agent, range_square: float) -> float:
        if agent != self:
            ...
            dist_sq = abs_sq(
                (self._position[0] - agent_pos[0], self._position[1] - agent_pos[1])
            )
    ...
    def insert_obstacle_neighbor(self, obstacle, range_square: float):
        ...
        dist_sq: float = dist_sq_point_line_segment(
            obstacle.get_point(), next_obstacle.get_point(), self._position
        )
        if dist_sq < range_square:
    ...
    def update(self, delta_time: float, move_agents: bool = True):
        ...
        if move_agents:
            self._position = (
                self._position[0] + self._velocity[0] * delta_time,
