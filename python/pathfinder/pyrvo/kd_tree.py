...
from pathfinder.pyrvo.utilities import MAX_LEAF_SIZE, RVO_EPSILON, abs_sq, left_of
...
class ObstacleTreeNode:
    ...
    def to_string(self):
        return (
            "<"
            + str(self._obstacle.get_id())
            + ">:["
            + (self._left.to_string() if self._left is not None else "None")
            + "-"
            + (self._right.to_string() if self._right is not None else "None")
            + "]"
        )
...
class KDTree:
    ...
    def build_agent_tree(self, force_update: bool):
        if force_update:
            ...
            self._agent_tree = [None] * (
                2 * len(self._agents) - 1 if len(self._agents) > 0 else 0
            )
        # start building
    ...
    def _build_agent_tree_recirsive(self, begin: int, end: int, node: int):
        ...
        for i in range(begin + 1, end):
            self._agent_tree[node]._max_x = max(
                self._agent_tree[node]._max_x, self._agents[i].get_position()[0]
            )
            self._agent_tree[node]._min_x = min(
                self._agent_tree[node]._min_x, self._agents[i].get_position()[0]
            )

            self._agent_tree[node]._max_y = max(
                self._agent_tree[node]._max_y, self._agents[i].get_position()[1]
            )
            self._agent_tree[node]._min_y = min(
                self._agent_tree[node]._min_y, self._agents[i].get_position()[1]
            )

        if (end - begin) > MAX_LEAF_SIZE:
            is_vertical = (
                self._agent_tree[node]._max_x - self._agent_tree[node]._min_x
                > self._agent_tree[node]._max_y - self._agent_tree[node]._min_y
            )
            split_value = (
                0.5 * (self._agent_tree[node]._max_x + self._agent_tree[node]._min_x)
                if is_vertical
                else 0.5
                * (self._agent_tree[node]._max_y + self._agent_tree[node]._min_y)
            )
            ...
            while left < right:
                while (
                    left < right
                    and (
                        self._agents[left].get_position()[0]
                        if is_vertical
                        else self._agents[left].get_position()[1]
                    )
                    < split_value
                ):
                    left += 1
                while (
                    right > left
                    and (
                        self._agents[right - 1].get_position()[0]
                        if is_vertical
                        else self._agents[right - 1].get_position()[1]
                    )
                    >= split_value
                ):
                    right -= 1
                ...
                if left < right:
                    self._agents[left], self._agents[right - 1] = (
                        self._agents[right - 1],
                        self._agents[left],
                    )
                    left += 1
                    right -= 1
                ...
                if obstacle_i1 is not None:
                    ...
                    for j in range(len(obstacles)):
                        ...
                        if obstacle_j1 is not None:
                            ...
                            j1_left_of_i: float = left_of(
                                obstacle_i1.get_point(),
                                obstacle_i2.get_point(),
                                obstacle_j1.get_point(),
                            )
                            j2_left_of_i: float = left_of(
                                obstacle_i1.get_point(),
                                obstacle_i2.get_point(),
                                obstacle_j2.get_point(),
                            )
                            if (
                                j1_left_of_i >= -RVO_EPSILON
                                and j2_left_of_i >= -RVO_EPSILON
                            ):
                                left_size += 1
                            elif (
                                j1_left_of_i <= RVO_EPSILON
                            ...
                            ) >= (max(min_left, min_right), min(min_left, min_right)):
                                break
                    if (max(left_size, right_size), min(left_size, right_size)) < (
                        max(min_left, min_right),
            ...
            if obstacle_i1 is not None:
                ...
                for j in range(len(obstacles)):
                    ...
                    if obstacle_j1 is not None:
                        ...
                        j1_left_of_i = left_of(
                            obstacle_i1.get_point(),
                            obstacle_i2.get_point(),
                            obstacle_j1.get_point(),
                        )
                        j2_left_of_i = left_of(
                            obstacle_i1.get_point(),
                            obstacle_i2.get_point(),
                            obstacle_j2.get_point(),
                        )
                        ...
                        if (
                            ...
                            j1_left_of_i <= RVO_EPSILON and j2_left_of_i <= RVO_EPSILON
                        ):
                            ...
                            t: float = (v1[0] * v12[1] - v1[1] * v12[0]) / (
                                v1[0] * v22[1] - v1[1] * v22[0]
                            )
                            split_point = (
                                pj1[0] + t * (pj2[0] - pj1[0]),
                                pj1[1] + t * (pj2[1] - pj1[1]),
    ...
    def _query_agent_tree_recursive(
        self, agent: Agent, range_square: float, node: int
    ) -> float:
        if self._agent_tree[node]._end - self._agent_tree[node]._begin <= MAX_LEAF_SIZE:
            for i in range(self._agent_tree[node]._begin, self._agent_tree[node]._end):
                range_square = agent.insert_agent_neighbor(
                    self._agents[i], range_square
                )
        else:
            dist_sq_left: float = (
                (
                    max(
                        0.0,
                        self._agent_tree[self._agent_tree[node]._left]._min_x
                ...
                ** 2
                + (
                    max(
                        0.0,
                        agent.get_position()[0]
                        - self._agent_tree[self._agent_tree[node]._left]._max_x,
                    )
                )
                ** 2
                + (
                    max(
                        0.0,
                        self._agent_tree[self._agent_tree[node]._left]._min_y
                ...
                ** 2
                + (
                    max(
                        0.0,
                        agent.get_position()[1]
                        - self._agent_tree[self._agent_tree[node]._left]._max_y,
                    )
                )
                ** 2
            )
            dist_sq_right: float = (
                (
                    max(
                        0.0,
                        self._agent_tree[self._agent_tree[node]._right]._min_x
                        - agent.get_position()[0],
                    )
                )
                ** 2
                + (
                    max(
                        0.0,
                        agent.get_position()[0]
                        - self._agent_tree[self._agent_tree[node]._right]._max_x,
                    )
                )
                ** 2
                + (
                    max(
                        0.0,
                        self._agent_tree[self._agent_tree[node]._right]._min_y
                        - agent.get_position()[1],
                    )
                )
                ** 2
                + (
                    max(
                        0.0,
                        agent.get_position()[1]
                        - self._agent_tree[self._agent_tree[node]._right]._max_y,
                    )
                )
                ** 2
            )
            if dist_sq_right > dist_sq_left:
                if dist_sq_left < range_square:
                    range_square = self._query_agent_tree_recursive(
                        agent, range_square, self._agent_tree[node]._left
                    )
                    if dist_sq_right < range_square:
                        range_square = self._query_agent_tree_recursive(
                            agent, range_square, self._agent_tree[node]._right
                        )
            else:
                if dist_sq_right < range_square:
                    range_square = self._query_agent_tree_recursive(
    ...
    def _query_obstacle_tree_recursive(
        self, agent: Agent, range_square: float, node: ObstacleTreeNode
    ):
        if node is None:
            return None
        else:
            ...
            agent_left_of_line: float = left_of(
                obstacle_01.get_point(), obstacle_02.get_point(), agent.get_position()
            )

            self._query_obstacle_tree_recursive(
                agent,
                range_square,
                node.get_left() if agent_left_of_line >= 0.0 else node.get_right(),
            )

            p1 = obstacle_01.get_point()
            p2 = obstacle_02.get_point()
            dist_sq_line: float = (agent_left_of_line**2) / abs_sq(
                (p2[0] - p1[0], p2[1] - p1[1])
            )
            if dist_sq_line < range_square:
                ...
                self._query_obstacle_tree_recursive(
                    agent,
                    range_square,
                    node.get_right() if agent_left_of_line >= 0.0 else node.get_left(),
                )
    ...
    def _query_visibility_recursive(
        self,
        ...
        if node is None:
            ...
            if q1_left_of_i >= 0.0 and q2_left_of_i >= 0.0:
                return self._query_visibility_recursive(
                    q1, q2, radius, node.get_left()
                ) and (
                    (
                        (q1_left_of_i**2) * inv_length_i >= radius**2
                        and (q2_left_of_i**2) * inv_length_i >= radius**2
                    )
                    or self._query_visibility_recursive(
                        q1, q2, radius, node.get_right()
                    )
                )
                ...
                return self._query_visibility_recursive(
                    ...
                    or self._query_visibility_recursive(q1, q2, radius, node.get_left())
                )
            elif q1_left_of_i >= 0.0 and q2_left_of_i <= 0.0:
                return self._query_visibility_recursive(
                    q1, q2, radius, node.get_left()
                ) and self._query_visibility_recursive(q1, q2, radius, node.get_right())
            else:
                ...
                return (
                    point_01_left_of_q * point_02_left_of_q >= 0.0
                    and (point_01_left_of_q**2) * inv_length_q > radius**2
                    and (point_02_left_of_q**2) * inv_length_q > radius**2
                    and self._query_visibility_recursive(
                        q1, q2, radius, node.get_left()
                    )
                    and self._query_visibility_recursive(
                        q1, q2, radius, node.get_right()
