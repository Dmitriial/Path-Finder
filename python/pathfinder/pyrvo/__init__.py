...
def add_agent_default(simulator, position: Tuple[float, float]) -> int:
    """
...
def delete_agent(simulator, agent_indexes: List[int]):
    ...
    if IMPORT_BINARY:
        raise NotImplementedError(
            "delete agent is not implemented in binary RVO2 library"
        )
    else:
        simulator.delete_agents(agent_indexes)


def add_obstacle(simulator, vertices: List[Tuple[float, float]]) -> int:
    """
...
def query_visibility(
    simulator, point_01: Tuple[float, float], point_02: Tuple[float, float]
) -> bool:
    if IMPORT_BINARY:
...
def simulate(simulator, delta_time: float, move_agents: bool = True):
...
# get agent methods
# -------------------------------------
def get_agent_agent_neighbor(simulator, agent_index: int, neighbor_index: int) -> int:
    if IMPORT_BINARY:
...
def get_agent_max_neighbors(simulator, agent_index: int) -> int:
    if IMPORT_BINARY:
...
def get_agent_max_speed(simulator, agent_index: int) -> float:
    if IMPORT_BINARY:
...
def get_agent_neighbor_dist(simulator, agent_index: int) -> float:
    if IMPORT_BINARY:
...
def get_agent_num_agent_neighbors(simulator, agent_index: int) -> int:
    if IMPORT_BINARY:
...
def get_agent_num_obstacle_neighbors(simulator, agent_index: int) -> int:
    if IMPORT_BINARY:
...
def get_agent_obstacle_neighbor(
    simulator, agent_index: int, neighbor_index: int
) -> int:
    if IMPORT_BINARY:
...
def get_agent_time_horizon(simulator, agent_index: int) -> float:
    if IMPORT_BINARY:
...
def get_agent_time_horizon_obst(simulator, agent_index: int) -> float:
    if IMPORT_BINARY:
...
def get_agent_velocity(simulator, agent_index: int) -> Tuple[float, float]:
    if IMPORT_BINARY:
...
def set_agent_max_neighbors(simulator, agent_index: int, max_neighbors: int):
    if IMPORT_BINARY:
...
def set_agent_max_speed(simulator, agent_index: int, max_speed: float):
    if IMPORT_BINARY:
...
def set_agent_neighbor_dist(simulator, agent_index: int, neighbor_dist: float):
    if IMPORT_BINARY:
...
def set_agent_pref_velocity(simulator, agent_index: int, velocity: Tuple[float, float]):
...
def set_agent_radius(simulator, agent_index: int, radius: float):
    if IMPORT_BINARY:
...
def set_agent_time_horizon(simulator, agent_index: int, time_horizon: float):
    if IMPORT_BINARY:
...
def set_agent_time_horizon_obst(simulator, agent_index: int, time_horizon_obst):
    if IMPORT_BINARY:
...
def set_agent_velocity(simulator, agent_index: int, velocity: Tuple[float, float]):
    if IMPORT_BINARY:
